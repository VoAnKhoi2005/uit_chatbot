...
Single FastAPI app, single LLM client (llm.client.LLMClient - generic
OpenAI-protocol, configured via LLM_BASE_URL/LLM_API_KEY/LLM_MODEL). This
used to be split into main_grop.py (Groq) and main_gpt.py (OpenAI); now that
one client talks to either endpoint the same way, there's one entry point.
"""
...
logger.info("Initializing LLM client and chat pipeline")
llm_client = LLMClient()
logger.info("LLM client configured: base_url=%s model=%s", llm_client.base_url, llm_client.model)
...
async def get_document(doc_id: str):
    ...
    logger.error("[PDF API] PDF file not found: filename=%s path=%s", filename, pdf_path)
